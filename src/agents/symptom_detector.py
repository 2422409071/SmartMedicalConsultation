...
class SymptomResult(BaseModel):
    ...
    symptoms: list[str] = Field(description="List of detected symptoms (consider the whole conversation)")
...
SYSTEM_PROMPT = """你是一个专业的症状分析专家。请结合【对话历史】与【本轮用户输入】，提取用户提到的全部症状。

重要：
- 必须综合历史对话：如果本轮只是"那挂什么科""还要看吗"等追问/承接句，而历史里已描述过症状，请把历史中的症状一并提取出来。
- 使用标准医学术语。
- 判断整体严重程度，并检测是否为急症。
...
历史：用户：我左眼疼，上眼皮肿了 / 助手：…
本轮：那应该挂什么科
症状：["左眼疼痛", "上眼睑肿胀"]   ← 从历史中提取
严重程度：medium
是否急症：false

请分析下面的对话与本轮输入，提取症状信息。"""
